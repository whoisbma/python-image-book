...
class LKTracker(object):
	...
	def draw(self):
		"""    Draw the current image with points using OpenCV's own drawing functions. Press ant key to close window."""
	...
	def track(self):
		...
		for i in range(len(self.imnames)):
			...
			# create a copy in RGB
			f = array(self.features).reshape(-1,2)
			im = cv2.cvtColor(self.images, cv2.COLOR_BGR2RGB)
			yield im,f

imnames = ['images/bt.003.pgm', 'images/bt.002.pgm', 'images/bt.001.pgm', 'images/bt.000.pgm']
